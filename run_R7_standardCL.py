"""
4-View R7 Baseline — DLPFC 12 Slices (Standard ContrastiveLoss, NO ICL)
========================================================================
Control experiment: same code as R7+ICL but using standard InfoNCE.
If this reproduces R7's ~0.5537, then ICL is the cause of regression.
"""
...
# PATCH: Override spCLUE to use standard ContrastiveLoss instead of ICL
from spCLUE.loss import ContrastiveLoss, ClusterLoss, MSELoss
...
print(f"4-View {ROUND}: R7 architecture but standard InfoNCE (NO ICL)")
print(f"   V1=spatial(uniform), V2=spatial(uniform indep)")
print(f"   V3=expr(uniform),    V4=expr(spatial-prior)")
print(f"   Fusion=4-way Attention, CL=V1<->V3 + V2<->V4 (STANDARD InfoNCE)")
...
for sample_name in samples_list:
    ...
    try:
        ...
        # OVERRIDE: Replace ICL with standard ContrastiveLoss
        spCLUE_model.instance_crit = ContrastiveLoss()
