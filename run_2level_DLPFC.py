"""
Two-Level Fusion — DLPFC 12 Slices
===================================
Framework: 4 views → 2-level fusion → Recon + CL
  L1: V1+V3 → Z_spa,  V2+V4 → Z_expr  (within-topology denoising)
  L2: Z_spa+Z_expr → Z_fuse            (cross-topology integration)

Loss: ICL+ClusterCL+Recon (same as R7 proven config)
kappa=0.05/pair (total=0.10)
"""
...
print(f"Two-Level Fusion {ROUND}: 4 views → 2-level hierarchical fusion")
print(f"  L1: ConcatProj within-topology (spatial pair + expr pair)")
print(f"  L2: ConcatProj cross-topology")
print(f"  CL: 4 pairs ICL + ClusterCL, Recon: MSE")
print(f"  kappa=0.03×2(cross) + 0.02×2(same) = 0.10")
...
for sample_name in samples_list:
    ...
    try:
        ...
        spCLUE_model = spCLUE.spCLUE(
            ...
            kappa=0.1,
            fusion_type="two_level",   # ← 2-level hierarchical fusion
        )
...
csv_filename = f"2level_{ROUND}_SpatialGDC_DLPFC_Results.csv"
